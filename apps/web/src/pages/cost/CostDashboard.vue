<template>
  <el-card shadow="never" class="page-card">
    ...
    <div class="toolbar">
      ...
      <el-select v-model="taskName" placeholder="选择任务（可选）" clearable class="ml8" style="width:240px" @change="load">
    ...
    <el-row :gutter="12" class="mt12">
      <el-col :span="24">
        <el-card shadow="hover">
          ...
          <div class="kpi-grid">
            <div class="kpi"><div class="kpi-title">项目总预算(BAC)</div><div class="kpi-value">{{ (kpis.projectTotalBudget||0).toFixed(2) }} {{ kpis.currency || currency }}</div></div>
            <div class="kpi"><div class="kpi-title">任务总预算</div><div class="kpi-value">{{ (kpis.taskTotalBudget||0).toFixed(2) }} {{ kpis.currency || currency }}</div></div>
            <div class="kpi"><div class="kpi-title">项目剩余预算</div><div class="kpi-value">{{ (kpis.projectRemainingBudget||0).toFixed(2) }} {{ kpis.currency || currency }}</div></div>
            <div class="kpi"><div class="kpi-title">项目总收入</div><div class="kpi-value">{{ (kpis.projectTotalRevenue||0).toFixed(2) }} {{ kpis.currency || currency }}</div></div>
            <div class="kpi"><div class="kpi-title">实际成本(AC)</div><div class="kpi-value">{{ (kpis.AC||0).toFixed(2) }} {{ kpis.currency || currency }}</div></div>
            <div class="kpi" :class="cpiClass"><div class="kpi-title">成本绩效指数(CPI)</div><div class="kpi-value">{{ (kpis.CPI||0).toFixed(2) }}</div></div>
            <div class="kpi"><div class="kpi-title">成本偏差(CV=EV-AC)</div><div class="kpi-value">{{ (kpis.CV||0).toFixed(2) }} {{ kpis.currency || currency }}</div></div>
            <div class="kpi"><div class="kpi-title">完工尚需估算(ETC)</div><div class="kpi-value">{{ (kpis.ETC||0).toFixed(2) }} {{ kpis.currency || currency }}</div></div>
            <div class="kpi"><div class="kpi-title">完工总估算(EAC)</div><div class="kpi-value">{{ (kpis.EAC||0).toFixed(2) }} {{ kpis.currency || currency }}</div></div>
          </div>
        </el-card>
      </el-col>
    </el-row>
    <el-row :gutter="12">
      <el-col :span="12">
        <el-card shadow="hover">
          ...
          <div class="mt12">
            ...
            <div class="overall">综合得分：<span class="overall-score">{{ overallScore.toFixed(2) }}</span></div>
      ...
      <el-col :span="12">
        <el-card shadow="hover">
          <div class="sub-title">度量逻辑</div>
          <el-table :data="metricRows" size="small" border>
            <el-table-column prop="name" label="维度" />
            <el-table-column prop="score" label="评分(0-5)" />
          </el-table>
          <div class="metric-notes">
            <div>单项成本控制率： (预算 − 实际) / 预算 × 100</div>
            <div>任务总成本控制率： 任务总成本 / 任务总预算 × 100（预算为四项之和）</div>
            <div>间接成本占比： 间接成本 / 直接成本 × 100</div>
          </div>
        </el-card>
    ...
    <el-row :gutter="12" class="mt12">
      <el-col :span="12">
        <el-card shadow="hover">
          <div class="sub-title">成本分类分布</div>
    ...
    <el-row :gutter="12" class="mt12">
      <el-col :span="24">
        <el-card shadow="hover">
          <div class="sub-title">成本列表（按任务汇总）</div>
          ...
          <div class="overall">合计：<span class="overall-score">{{ total.toFixed(2) }} {{ currency }}</span></div>
        </el-card>
      </el-col>
    </el-row>
  </el-card>
...
async function load() {
  ...
  for (const r of rows.value) {
    const t = r.taskName || ''
    if (!t) continue
    m.set(t, (m.get(t) || 0) + Number(r.totalCost || 0))
  }
  aggRows.value = Array.from(m.entries()).map(([taskName, totalCost]) => ({ taskName, totalCost }))
...
async function loadRadar() {
  ...
  metricRows.value = [
    { name: '材料成本控制率', score: Number(d.scores.materials || 0).toFixed(2) },
    { name: '人工成本控制率', score: Number(d.scores.labor || 0).toFixed(2) },
    { name: '设备成本控制率', score: Number(d.scores.equipment || 0).toFixed(2) },
    { name: '服务成本控制率', score: Number(d.scores.services || 0).toFixed(2) },
    { name: '任务总成本控制率', score: Number(d.scores.taskTotal || 0).toFixed(2) },
    { name: '间接成本占比', score: Number(d.scores.indirectRatio || 0).toFixed(2) }
  ]
...
function onRowClick(row: any) { if (row?.taskName) taskName.value = row.taskName }
