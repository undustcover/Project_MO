<template>
  <el-card shadow="never" class="page-card">
    ...
    <div class="toolbar">
      ...
      <el-select v-model="statusFilter" placeholder="状态筛选" class="ml8" style="width:160px" @change="loadRecords">
...
async function loadRecords() {
  ...
  u.searchParams.set('projectId', String(projectId.value))
  if (statusFilter.value) u.searchParams.set('status', statusFilter.value)
...
function renderCharts(compareData: any[] | null) {
  ...
  const sev = Object.keys(sevCount).map(k => ({ name: k, value: sevCount[k] }))
  const types = Object.keys(typeCount); const typeVals = types.map(k => typeCount[k])
  const dates = Object.keys(dateCount).sort(); const dateVals = dates.map(k => dateCount[k])
